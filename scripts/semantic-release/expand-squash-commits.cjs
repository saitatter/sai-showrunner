...
function parseConventionalCommitLine(commit, line, index, source = "body") {
	...
	return {
		...commit,
		hash: source === "body" && commit.hash ? commit.hash : `${commit.hash || "squash"}-${source}-${index}`,
		message: header,
		header,
		subject,
		type,
...
async function createExpandedContext(context) {
	const expandedContext = {
		...context,
		commits: Array.isArray(context.commits) ? [...context.commits] : context.commits,
	}
	await applyExpansion(expandedContext)
...
module.exports = {
	async analyzeCommits(pluginConfig = {}, context) {
		...
		const expandedContext = await createExpandedContext(context)
		return analyzeCommits(pluginConfig.analyzerOptions || {}, expandedContext)
	},
	async generateNotes(pluginConfig = {}, context) {
		const { generateNotes } = await import("@semantic-release/release-notes-generator")
		const expandedContext = await createExpandedContext(context)
		return generateNotes(pluginConfig.notesOptions || {}, expandedContext)
	},
	expandSquashCommits,
