...
import { isShowRunner, isSatellite } from "../util/init-mode"
...
export const SatelliteResources = Service(
	class {
		...
		async createRemoteResourceSlot(id: string, resourceType: string, name: string) {
			if (!isShowRunner()) throw new Error("This is ShowRunner side only")
		...
		async bindResourceSlot<T extends ResourceBase>(slotId: string, resource: T) {
			...
			const connnectionId = SatelliteService.getInstance().getShowRunnerConnection()
