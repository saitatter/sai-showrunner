...
export const useSatelliteConnection = defineStore("satellite-connection", () => {
	...
	function getConnection(request: SatelliteConnectionRequestConfig) {
		const service = initStore.isShowRunner ? request.satelliteService : request.ShowRunnerService
		const id = initStore.isShowRunner ? request.satelliteId : request.ShowRunnerId
	...
	function startDashboardConnection(config: SatelliteConnectionRequestConfig) {
		const self = createConnection(config.ShowRunnerService, config.ShowRunnerId, config.dashId)
	...
	async function handleClientConnection(desc: SatelliteConnectionRequest) {
		...
		self.connection.onicecandidate = async (ev) => {
			...
			const candidateMsg: SatelliteConnectionICECandidate = {
				...
				ShowRunnerService: desc.ShowRunnerService,
				ShowRunnerId: desc.ShowRunnerId,
				dashId: desc.dashId,
				side: "ShowRunner",
				candidate: ev.candidate.toJSON(),
			}
	...
	async function connectToShowRunner(request: SatelliteConnectionRequestConfig) {
