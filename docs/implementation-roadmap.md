...
- [ ] Keep shader graph nodes low-level and composable; avoid one-shot scene/terrain generator nodes.
- [ ] Add low-level shader primitives: ridged FBM, turbulence, curl noise, cellular F1/F2, bias/gain, posterize, and remap helpers.
- [ ] Add low-level material helpers: normal transforms, triplanar-style coordinates, layer masks, and simple BRDF pieces.
