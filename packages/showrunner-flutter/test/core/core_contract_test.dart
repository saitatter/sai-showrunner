...
void main() {
  ...
  test('executes compiled subgraph parameter and output wires', () async {
    final automation = AutomationData(
      graph: const AutomationGraph(
        nodes: [
          GraphNode(id: 'producer', type: 'action', x: 0, y: 0),
          GraphNode(
            id: 'call',
            type: 'subgraphCall',
            x: 100,
            y: 0,
            data: {'subgraphId': 'echo', 'inputs': {}},
          ),
          GraphNode(id: 'consumer', type: 'action', x: 200, y: 0),
        ],
        edges: [
          GraphEdge(id: 'to-call', from: 'producer', to: 'call'),
          GraphEdge(id: 'to-consumer', from: 'call', to: 'consumer'),
        ],
      ...
      subgraphs: [
        SubgraphDefinition(
          ...
          nodes: const [GraphNode(id: 'echo-node', type: 'action', x: 0, y: 0)],
          edges: const [],
          dataWires: const [
            DataWire(
              id: 'parameter-wire',
          ...
          entryNodeId: 'echo-node',
        ),
      ],
    );
    RuntimeMap? receivedConfig;
    final result = await const DartCompiledGraphRuntime().execute(
      graph: const DartGraphCompiler().compileAutomation(automation),
      context: EvaluationContext(),
      action: (instruction, config, context) async {
        if (instruction.nodeId == 'producer') {
          return {
            'payload': {'message': 'compiled wire'},
          };
        }
        if (instruction.nodeId == 'echo-node') {
          return {'echoed': config['text']};
        }
    ...
    expect(result.completed, isTrue);
    expect(receivedConfig, {'text': 'compiled wire'});
    expect(result.nodeResults['call'], {'message': 'compiled wire'});
  });
    ...
    () async {
      final automation = AutomationData.fromJson({
        ...
        'graph': {
          'entryNodeId': 'call',
          'nodes': [
            {'id': 'call', 'type': 'subgraphCall', 'subgraphId': 'greeting'},
            {
              'id': 'done',
              'type': 'action',
              'config': {'label': 'done'},
      ...
      final result = await const DartGraphRuntime().execute(
        ...
        subgraphs: {
          for (final subgraph in automation.subgraphs) subgraph.id: subgraph,
        },
        context: EvaluationContext(),
      ...
      expect(result.completed, isTrue);
      expect(executed, ['hello', 'done']);
    },
  );

  test('limits recursive subgraph calls in both runtimes', () async {
    final recursive = SubgraphDefinition(
      id: 'recursive',
      name: 'Recursive',
      nodes: const [
        GraphNode(
          ...
          x: 0,
          y: 0,
          data: {'subgraphId': 'recursive'},
        ),
      ],
      edges: const [],
      entryNodeId: 'call-self',
    );
    final automation = AutomationData(
      graph: const AutomationGraph(
        nodes: [
    ...
    await expectLater(
      const DartGraphRuntime(maxDepth: 3).execute(
        graph: automation.graph,
        context: EvaluationContext(),
        subgraphs: subgraphs,
    ...
    await expectLater(
      const DartCompiledGraphRuntime(maxDepth: 3).execute(
        graph: const DartGraphCompiler().compileAutomation(automation),
        context: EvaluationContext(),
        action: (instruction, config, context) async => null,
      ),
      throwsA(isA<StateError>()),
    );
  });
  ...
  test('binds subgraph parameters and exposes output wires', () async {
    RuntimeMap? receivedConfig;
    final graph = AutomationGraph(
      nodes: const [
        GraphNode(id: 'producer', type: 'action', x: 0, y: 0),
    ...
    final result = await const DartGraphRuntime().execute(
      ...
      action: (node, config, context) async {
        if (node.id == 'producer') {
          return {
            'payload': {'message': 'wired'},
          };
        }
        if (node.id == 'echo-node') return {'echoed': config['text']};
        receivedConfig = config;
        return null;
      },
    );
    ...
    expect(result.completed, isTrue);
    expect(receivedConfig, {'text': 'wired'});
    expect(result.nodeResults['call'], {'message': 'wired'});
  });
  ...
  test(
    'compiled runtime normalizes primitive results and applies mappings',
    () async {
      const graph = AutomationGraph(
        nodes: [
          GraphNode(
            ...
            y: 0,
            data: {
              'resultMapping': {'_result': 'mappedValue'},
            },
          ),
        ],
        entryNodeId: 'action-1',
      ...
      final result = await const DartCompiledGraphRuntime().execute(
        graph: compiled,
        context: EvaluationContext(),
        action: (instruction, config, context) async => 42,
      );

      expect(result.nodeResults['action-1'], {'_result': 42});
      expect(result.contextState, {'mappedValue': 42});
    },
  );
