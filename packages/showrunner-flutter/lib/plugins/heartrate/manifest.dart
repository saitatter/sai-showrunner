...
DartPluginManifest createHeartRatePlugin(
  ...
  actions: [
    ActionSpec<HeartRateEmptyConfig, void>(
      ...
      configCodec: heartRateEmptyConfigCodec,
  ...
  triggers: service.eventHub == null
      ? const []
      : [
          TriggerSpec<HeartRateEmptyConfig, HeartRateDeviceConnectedEvent>(
            pluginId: PluginId('heartrate'),
...
Stream<T> _mapEvents<T>(
  DartPluginEventHub hub,
  String eventId,
  T Function(RuntimeMap) decode,
) => hub.stream(eventId).map(decode);

Stream<HeartRateThresholdEvent> _emptyThresholdTrigger() =>
    const Stream<HeartRateThresholdEvent>.empty();

Stream<HeartRateThresholdEvent> _thresholdEvents(
  DartPluginEventHub hub,
  HeartRateThresholdConfig config, {
  required bool above,
}) {
  final threshold = config.threshold;
  final hysteresis = config.hysteresis;
  final cooldown = Duration(seconds: config.cooldownSeconds);
  bool? latched;
  DateTime? lastFired;
  late final StreamSubscription<RuntimeMap> subscription;
  late final StreamController<HeartRateThresholdEvent> controller;
  controller = StreamController<HeartRateThresholdEvent>(
    onListen: () {
      subscription = hub.stream(HeartRateEventIds.measurement).listen((event) {
        ...
        lastFired = now;
        controller.add(
          HeartRateThresholdEvent(bpm: value, threshold: threshold),
        );
      });
    },
